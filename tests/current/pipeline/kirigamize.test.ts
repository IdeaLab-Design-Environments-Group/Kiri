...
describe("pipeline/kirigamize", () => {
  beforeEach(() => {
    ...
    mocks.emitFkld.mockReturnValue({ file_creator: "Kiri" });
  ...
  it("replans around the worst source vertex on attempt 3 and adopts the better retry", () => {
    ...
    mocks.emitFkld
      .mockReturnValueOnce({ file_creator: "Kiri", frame_title: "first" })
      .mockReturnValueOnce({ file_creator: "Kiri", frame_title: "retry" });
    mocks.verifyFold
  ...
  it("parses text input before delegating to the main pipeline", () => {
    ...
    expect(result.fkld.file_creator).toBe("Kiri");
  });
});
